...
describe("six-step trade pipeline", () => {
  ...
  it("hard-blocks a minor on every market", async () => { const kid = { ...clean, agentId: "ans://kid.demo", birthday: "2021-03-15" }; const store = new MemoryStore(kid, market); const result = await new TradePipeline(store, ans, payments, "pool", 60).execute(signed(kid)); expect(result.decision).toBe("blocked"); expect(result.reasons.join(" ")).toContain("under 18"); expect(store.pool.outcomeA).toBe(0); });
