...
revoke all on function public.apply_cleared_trade(uuid,text,numeric) from public;
revoke all on function public.append_trade_decision(uuid,text,text[],timestamptz) from public;
revoke all on function public.append_decision_v2(uuid,text,text[],jsonb) from public;
revoke all on function public.prepare_trade(jsonb,text,text[],boolean) from public;
revoke all on function public.finish_payment(uuid,text,text,text) from public;
revoke all on function public.prepare_resolution(uuid,text,text) from public;
revoke all on function public.claim_payout(uuid) from public;

-- Only the migration owner (the server connection role) can call mutation functions.
revoke all on public.agent_identities,public.markets,public.trade_nonces,public.trade_decisions,public.payment_jobs from public;
revoke all on sequence public.trade_decisions_sequence_seq from public;
