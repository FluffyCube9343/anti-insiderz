...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ agentId: agent.agentId, affiliations: agent.affiliations, publicKey: agent.publicKey, registeredAt: agent.registeredAt, email: agent.email ?? null, birthday: agent.birthday ?? null, role: agent.role ?? null });
  } catch (e) {
