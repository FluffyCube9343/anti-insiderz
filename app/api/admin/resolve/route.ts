...
export async function POST(request:Request) {
  ...
  try {const pool=nessieConfig().pool;
    try {await store().db.query("select public.prepare_resolution($1::uuid,$2,$3)",[b.marketId,b.outcome,pool]);}
    catch{return NextResponse.json({error:"Resolution refused. Reconcile pending trades, verify the position ledger, and confirm the market outcome has not already been set."},{status:409});}
